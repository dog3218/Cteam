			...
			<ul>
				<li>
					<select name="search" class='w-px90'>
						...
						<option value="title" ${page.search eq 'title' ? 'selected' : '' }>공연명</option>
						<option value="content" ${page.search eq 'content' ? 'selected' : '' }>장르</option>
	...
	<c:if test="${page.viewType eq 'grid' }">
		...
		<!-- 조회된 목록이 없을 경우 정보 표시 -->
			...
			<c:forEach items="${page.list }" var = "vo">
				<li>
				<c:if test="${vo.filepath ne null}"><img src= '/app/imgs/${vo.filepath}' class='board-img' /></c:if>
				<c:if test="${vo.filepath eq null}"><img src= 'imgs/notyet.jpg' class='board-img' /></c:if>
					<!-- "'\" fhaskl\'dsafasdfa\'jdfhlasd\"'" -->
