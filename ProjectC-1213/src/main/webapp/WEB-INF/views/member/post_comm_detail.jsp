...
<style type="text/css">
	#popup { width: 600px; height: 600px; border: 2px solid #777; display: none; }
	.popup { width: 100%; height: 100%;}
	
	.comment { margin: 0 auto; padding-top: 30px; width: 500px;}
...
<%-- 		<td colspan="5" class="left">${fn:substring(vo.filename1, fn:indexOf(vo.filename1, '_') +1, fn:length(vo.filename1)) } --%>
			<c:if test="${! empty vo.filename1 }">
				<a id = 'preview'></a>
				<a href='download.co?id=${vo.id }'><i class="fas fa-download font-img"></i></a>		
			</c:if>
		</td>
	</tr>
</table>
...
<div class='btnSet'>
	...
	<c:if test="${loginInfo.email eq vo.writer }">		
		<a class='btn-fill' onclick= '$("form").attr("action", "modify_comm.my"); 
		$("form").submit()' >수정</a>
		<a class='btn-fill' onclick="if (confirm ('정말 삭제하시겠습니까?') )
		 { href='delete_comm.my?id=${vo.id }'  } ">삭제</a>
	</c:if>
</div>
...
function comment_regist() {
	...
	$.ajax ({
		...
		, data : { pno:${vo.id} , content:$('#comment').val()  }
...
/* function go_comm_list(){
		if(${state}.equals("mypage"))
			location="mypage.user";
		else{
			$("form").submit();
		}
	} */
